...
median_log2_by_group <- tpm_long %>%
  group_by(bintype, Condition) %>%
  summarise(
    median_log2TPM = median(log2TPM, na.rm = TRUE),
    .groups = "drop"
  )

median_log2_by_group

# library(dplyr)
# 
# TPM_region_cond <- tpm_long %>%
#   group_by(gene_name, bintype, Condition) %>%
#   summarise(
#     TPM_median = median(TPM, na.rm = TRUE),
#     .groups = "drop"
#   )
# 
# TPM_region_cond <- TPM_region_cond %>%
#   mutate(
#     Group = paste(bintype, Condition, sep = "_"),
#     log2TPM = log2(TPM_median + 1)
#   )
# 
# p_all <- ggplot(TPM_region_cond,
#                 aes(x = Group, y = log2TPM, fill = Group)) +
#   geom_boxplot(
#     width = 0.65,
#     color = "black",
#     outlier.size = 0.6,
...
# print(p_all)
# 
# median_log2_by_group <- TPM_region_cond %>%
#   group_by(bintype, Condition) %>%
#   summarise(
#     median_log2TPM = median(log2TPM, na.rm = TRUE),
#     .groups = "drop"
